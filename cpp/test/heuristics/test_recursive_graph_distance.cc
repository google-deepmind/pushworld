...
BOOST_AUTO_TEST_CASE(test_trivial) {
  ...
  // result.
  State s = puzzle->getInitialState();
  BOOST_TEST(rgd.estimate_cost_to_goal(s) == 2);
  BOOST_TEST(rgd.estimate_cost_to_goal(s) == 2);

  s = puzzle->getNextState(s, RIGHT);
  BOOST_TEST(rgd.estimate_cost_to_goal(s) == 3);
  BOOST_TEST(rgd.estimate_cost_to_goal(s) == 3);

  s = puzzle->getNextState(s, UP);
...
BOOST_AUTO_TEST_CASE(test_multiple_goals) {
  ...
  const State s0 = puzzle->getInitialState();
  BOOST_TEST(rgd.estimate_cost_to_goal(s0) == 4);

  State s = puzzle->getNextState(s0, LEFT);
  BOOST_TEST(rgd.estimate_cost_to_goal(s) == 4);

  s = puzzle->getNextState(s0, RIGHT);
  BOOST_TEST(rgd.estimate_cost_to_goal(s) == 4);

  s = puzzle->getNextState(s0, UP);
  BOOST_TEST(rgd.estimate_cost_to_goal(s) == 6);

  s = puzzle->getNextState(s0, DOWN);
  BOOST_TEST(rgd.estimate_cost_to_goal(s) == 6);
}
...
BOOST_AUTO_TEST_CASE(test_transitive_pushing) {
  ...
  BOOST_TEST(h0.estimate_cost_to_goal(puzzle->getInitialState()) == 3);
  // Check that internal caching doesn't change the result.
  BOOST_TEST(h0.estimate_cost_to_goal(puzzle->getInitialState()) == 3);

  RecursiveGraphDistanceHeuristic h1(puzzle);
  BOOST_TEST(h1.estimate_cost_to_goal(puzzle->getInitialState()) == 4);
  // Check that internal caching doesn't change the result.
  BOOST_TEST(h1.estimate_cost_to_goal(puzzle->getInitialState()) == 4);
  ...
  BOOST_TEST(h2.estimate_cost_to_goal(puzzle->getInitialState()) == 9);
  // Check that internal caching doesn't change the result.
  BOOST_TEST(h2.estimate_cost_to_goal(puzzle->getInitialState()) == 9);
  ...
  BOOST_TEST(h8.estimate_cost_to_goal(puzzle->getInitialState()) == 13);
  // Using tool: 1 + 3 + 2 = 6
  RecursiveGraphDistanceHeuristic h9(puzzle, false);
  BOOST_TEST(h9.estimate_cost_to_goal(puzzle->getInitialState()) == 6);
}
